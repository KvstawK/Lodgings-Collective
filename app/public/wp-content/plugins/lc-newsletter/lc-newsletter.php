...
//			require_once( LC_NEWSLETTER_PATH . 'inc/send-email-to-subscribers.php' );
			require_once( LC_NEWSLETTER_PATH . 'inc/create-newsletter-page-in-theme.php' );
		...
		public static function activate() {
			...
			lc_create_newsletter_page();

			$newsletter_page = get_page_by_title('Lodgings Collective Newsletter');
			if ($newsletter_page) {
				update_post_meta($newsletter_page->ID, '_lc_newsletter_email_sent', false);
			}
		}
		...
		public static function deactivate() {
			...
			$newsletter_page = get_page_by_title('Lodgings Collective Newsletter');
			if ($newsletter_page) {
				delete_post_meta($newsletter_page->ID, '_lc_newsletter_email_sent');
			}

			lc_delete_newsletter_page();
...
if(class_exists('LC_Newsletter')) {
	...
	register_activation_hook(__FILE__, 'lc_newsletter_unsubscribe_page_activate');
	register_deactivation_hook(__FILE__, 'lc_newsletter_unsubscribe_page_deactivate');
