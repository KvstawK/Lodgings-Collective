...
function html_form_code() {
	...
	echo '<form action="' . esc_url( $_SERVER['REQUEST_URI'] ) . '" method="post" class="lc-contact-forms-form">';
	...
	echo '</p>';
	echo '<p>';
	echo '<input type="text" aria-label="' . esc_html__('Please enter your subject', 'lc-contact-forms') . '" name="lc_contact_form-subject" placeholder="' . esc_html__('Subject', 'lc-contact-forms') . '" pattern="[\u0370-\u03FF\u1F00-\u1FFF\a-zA-Z ]+" value="' . ( isset( $_POST["lc_contact_form-subject"] ) ? esc_attr( $_POST["lc_contact_form-subject"] ) : '' ) . '" size="40" />';
	echo '</p>';
	echo '<p>';
	...
	echo '</p>';
	echo '<p><input class="btn" type="submit" name="lc_contact_form-submitted" value="' . esc_html__('Send', 'lc-contact-forms') . '"></p>';
	echo '</form>';
}
...
function deliver_mail() {
	...
	if ( isset( $_POST['lc_contact_form-submitted'] ) ) {
		...
		// Sanitize subject field and add domain name
		$subject = 'Contact form message from ' . $domain_name . ': ' . sanitize_text_field( $_POST["lc_contact_form-subject"] );
		...
		if ( wp_mail( $to, $subject, $message, $headers ) ) {
			...
			echo '<style>.contact__form-container-form { box-shadow: none; }</style>';
			echo '<div>';
			echo '<p class="headline-3 headline-3--orange">' . esc_html__('Thanks for contacting me, expect a response soon!', 'lc-contact-forms') . '</p>';
			echo '</div>';
		} else {
